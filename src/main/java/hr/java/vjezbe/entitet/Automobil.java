...
public class Automobil extends Artikl implements Vozilo, Serializable {
    ...
    public String tekstOglasa() {
        ...
        return String.format(
                "Naslov automobila: %s Opis automobila: %s Stanje automobila: %s Snaga automobila u kW: %s Izracun osiguranja automobila: %s Cijena automobila: %s", getNaslov(), getOpis(), getStanje().toString(), izracunajKw(snagaKs).setScale(2, RoundingMode.CEILING), tekstIzracunaOsiguranja, getCijena()
        );
    }
}
